...
class DatGenerator:
    ...
    def generate_dats(self, entries):
        ...
        entries : list(str)
            A list of entry IDs
        """
        entries = [entry for entry in entries if not os.path.isfile("dats/" + entry + ".dat")]
        futures = [self.executor.submit(self.get_reaction_ids_from_kgml, entry) for entry in entries]
...
def get_full_reaction(reaction_ids):
    ...
    for i, sublist in enumerate(split_into_smaller_sublist(reaction_ids, sublist_size)):
        print("Fetching reactions " + "+".join(
            sublist) + f" from the KEGG API... ({i + 1}/{math.ceil(len(reaction_ids) / sublist_size)})", end="")
...
def get_or_generate_dat(entry):
    ...
    isolate_reaction_id = lambda reaction_name: reaction_name.replace("rn:",
                                                                      "")  # isolate_reaction_id("rn:R00001") = "R00001"
